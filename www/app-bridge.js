...
(function () {
  ...
  var NOISE_REGEXES = [
    ...
    /^\d{4}[\.\-/]\d{1,2}[\.\-/]\d{1,2}/,
    /^\s*[\d.,:：、\-()（）\s]+\s*$/,           // 纯符号数字
    // v2.19 关键修复：JS 的 \W 不认汉字（汉字算"非字母数字"），旧正则 /^\s*[\W_]+\s*$/ 会把
    //   任何"纯中文题干行"（如"可计算的""模糊的"）误判为噪声整行删掉 → 提炼只剩 ABCD 选项！
    //   改为：只有"全非字母数字且全非汉字"的纯符号行才删（如 "!!!@@@"）
    /^\s*[^\u4e00-\u9fa5A-Za-z0-9_\s]+\s*$/,   // 纯符号（不含汉字/字母/数字）
    /^题型[:：]/,
    /^岗位[:：]/
  ];
  ...
  function extractQuestionText(raw) {
    ...
    //   2) 失败：firstOpt 之前的整段
    //   3) 再失败：无选项时用题号/关键词触发
    // v2.19 全新策略：不再信任脆弱的"行过滤+关键词"提炼。
    // 提炼出的题干如果中文字太少（<10 字）→ 判定提炼失败 → 直接返回去噪全文。
    // 这样"自动提炼"永远不会输出"只有选项"的坏结果。
    // 题干中文提取：从 stem 里把选项行（A./A、/A:）之前的部分视为题干
    function extractStemPart(s){
      var m = s.split(/\n/);
      var stemLines = [];
      for (var x=0;x<m.length;x++){
        if (/^\s*[A-Da-d]\s*[.、:：]\s*\S/.test(m[x]) || /^\s*[A-Da-d]\s+[^\s]/.test(m[x])) break;
        stemLines.push(m[x]);
      }
      return stemLines.join("\n");
    }
    // 中文字符计数
    function cjkCount(s){ var c=0; for(var z=0;z<s.length;z++){var ch=s[z];if(/[\u4e00-\u9fa5]/.test(ch))c++;} return c; }
    ...
    // ===== v2.19 关键：题干质量校验 =====
    // 提炼结果里，选项（A./A、/A: 起头的行）之前的中文部分必须足够长；
    // 否则（比如只有 "可计算的" 这种选项残片）判定提炼失败 → 用去噪全文
    var stemPart = extractStemPart(stem);
    var stemCJK = cjkCount(stemPart);
    var cleanedFull = cleaned.join("\n");
    if (stemCJK < 10) {
      // 提炼失败：题干太少（<10 个汉字）→ 回退全文（去噪后），保留所有题干+选项
      stem = cleanedFull;
    }
