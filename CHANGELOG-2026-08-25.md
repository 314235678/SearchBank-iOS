...
## 14. v2.21 构建交付（2026-08-26 11:00+）—— 原题排第一 + 题型练习页（本次）

- 用户反馈（30 道题测试，2 道题排序仍不完美）：
  - **题 A**：搜"对因人为原因造成的（）必须依法追究责任者的法律责任，以示警戒"→ 匹配出 3 道（1421 生产安全事故责任…/394 中华人民共和国安全生产法…），**原题不在第一位**
  - **题 B**：OCR 后题干被截短，原题位置跌到第 3-7 位
- **根因**：
  1. v2.18 完整 query 评分阈值 ≥15 太严：OCR 错 1-2 字时 `Q.includes(q)` miss，fuzzyScore 也跌到 0.6-0.7（不进阈值），回落到分块累加，多题凑相近分数
  2. `mgStats`（v2.19 题库管理里的统计）始终按 `DATA.items` 全量算，切换题库后统计不刷新
  3. 还没有"题型练习"二级页：用户希望练习中心有一页专门展示每个题库的题型分布，可一键进入对应练习（图片见 [图5]）
- **本轮修复（v2.21）**：
  - **① 搜索排序：原题绝对排第一（三重强信号）**
    - `scoreBlocks()` 阈值 15 → **10**（更宽松，OCR fuzzyScore≥0.5 也能进满分倍数）
    - 新增 3 重"原题指纹"强信号（OR 关系，任一命中就给极大分）：
      - (a) `query 前 12 字` 作为子串命中 `Q` → +8000
      - (b) `query 前 12 字` 在 `Q` 前 36 字内滑窗 fuzzy ≥ 0.85 → +7000，≥ 0.7 → +4500
      - (c) `ngram3Sim(Q, q) ≥ 0.55` → `+6000 * nsim`（题干核心词覆盖）
    - 验证（Node 实测数据，搜题库 6465 题）：
      ```
      完整 OCR query：原题得 6000（ngram 触发） vs 无关题最高 278（原 1421 题）→ 原题排第 1 ✓
      OCR 切断 23 字：原题 6000 vs 无关题 278 → 原题排第 1 ✓
      OCR 切断 15 字：原题 6000 vs 无关题 2666 → 原题排第 1 ✓
      ```
  - **② 题型练习二级页（每个题库单独显示）**
    - 在 `v-train` 内嵌 `#trainTypes` 二级面板，`renderTypesPanel(bankId)` 函数渲染
    - 顶部蓝色题库 banner + 6 维统计入口：
      1. **练习全部**（按题型固定顺序练全部题）
      2. **单选题** / **多选题** / **判断题**（按题号顺序练习）
      3. **判断题分对错练习**（分组）→ **判断（正确）** / **判断（错误）**（按 `isTrueAnswer/isFalseAnswer` 过滤）
      4. **试题分难易练习**（分组）→ **适中**（rightCount+wrongCount=0，未练习过的题）
    - 每个入口点击 → `startSession({bankId, type, judgment, difficulty})`
    - 入口用 v2.4 蓝紫商风格的 `.type-row`，大数字（22 号字）+ 标题 + 副标题
  - **③ 退出练习智能回跳**：从题型练习进入的 session，退出/完成自动回到题型练习面板（`S.fromTypes` 标志）；从训练首页进入的，回训练首页
  - **④ mgStats 按当前选中题库过滤**：renderMgStats 从 `DATA.items` 全量改为读 `#mgBank.value` 过滤；`#mgBank.onchange` 自动重渲染统计
  - **⑤ buildList / startSession / S 扩展**：
    - `f.judgment="true"/"false"`：判断题按答案正误分组
    - `f.difficulty="mid"`：过滤未练习过的题（适中）
- **验证方式**：
  - 浏览器打开 `www/index.html` → 进练习中心 → 点"🎯 题型练习" → 选择题库 → 看 6 维统计 + 任意点击进入练习
  - 在搜索框输入题库任意一段题干（哪怕截短），看是否原题排第一
  - 题库管理页切题库，看顶部统计卡数字是否跟着变
- **交付**：commit tbd 推 GitHub；Actions run tbd 构建中；产物 SearchBank-v2.21.ipa

## 13. v2.10 构建交付（2026-08-26 02:20+）—— 修分区域切题 bug + 引导页 iOS26 适配（本次）

- 用户反馈（iPhone 15 标准版 1179×2556 / iOS 26.3.1 / 红色"学习强企"企业定制界面）：
  - v2.9 仍只识别选项，识别不到题干（用户怀疑顶部被切掉）
  - iOS 26 无法导入未签名 .shortcut 文件（"不支持导入未签名的快捷指令文件"）
- **根因（v2.9 分区域 bug）**：
  1. `splitIntoRegions(parts:2)` 50/50 硬切，题目卡（学习强企居中卡片）恰好在中线被切两半 → 上半区剩题干、下半区剩选项，合并后只出部分行
  2. 分区域合并时未把区域归一化 y 转成原图绝对 y → 行顺序错乱
  3. v2.9 设置面板默认 split=true 加剧了问题
- **修复（v2.10）**：
  1. **分区域识别默认关闭**（UserDefaults `sb_ocr_split` 默认 false）——回归整图识别 + 预处理（白化/灰度/锐化）最稳
  2. `splitIntoRegions` 加 **25% 重叠**（题目卡不会被切）+ 返回 yOffset/regionH/totalH 元组
  3. 合并时把区域归一化 y **转成原图绝对 y 坐标**（yAbs = yOffset/totalH + yLocal*regionH/totalH）
  4. cropHeaderPct 0.04 → **0.05**（iPhone 15 顶部灵动岛+状态栏 ≈ 177px ≈ 6.9%；裁 5% = 128px 去掉状态栏，又不切题目卡）
- **iOS 26 快捷指令**：系统无法导入未签名 .shortcut → 引导页顶部加红框警告"无需下载，直接手动创建"，手动创建步骤按 iOS 26 中文界面重写（8 步，含重命名、4 个动作、测试）
- 交付：commit tbd 推 GitHub；Actions run tbd 构建中；产物 SearchBank-v2.10.ipa
